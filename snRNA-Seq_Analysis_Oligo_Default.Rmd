...
write.csv(oligos@assays$RNA@counts, file = paste0(data_dir, "/", "nueron_UMI_counts.csv"))
```
...
BNLx_1_Neurons.cellvect <- (oligos@meta.data) %>%
  filter(expt == '1_BNLx_1_Neurons') %>%
  rownames()
BNLx_2_Neurons.cellvect <- (oligos@meta.data) %>%
  filter(expt == '3_BNLx_2_Neurons') %>%
  rownames()
SHR_1_Neurons.cellvect <- (oligos@meta.data) %>%
  filter(expt == '5_SHR_1_Neurons') %>%
  rownames()
SHR_2_Neurons.cellvect <- (oligos@meta.data) %>%
...
write_csv(oligos.markers, file = paste0(data_dir, "/", "all_markers_all_clusters.csv"))
```